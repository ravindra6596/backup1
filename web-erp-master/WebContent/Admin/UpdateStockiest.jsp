...
<section class="content">
	<div class="box box-default">
		...
		Connection con = C3P0DataSource.getInstance().getConnection();
					ResultSet rs1=con.createStatement().executeQuery("SELECT *,DATE_FORMAT(druglicensevalidity, '%d-%m-%Y') as drug FROM stockiest INNER JOIN `stokiestcontact` ON `stokiestcontact`.`sid`=`stockiest`.`sid` WHERE `stockiest`.`sid`='"+sid+"'");
				//	ResultSet rs2=con.createStatement().executeQuery("select *, DATE_FORMAT(druglicensevalidity, '%d-%m-%Y') as drug from stockiest where sid='" + sid + "'");
					
				%>
		<form method="post" action="<%= rootpath %>StockiestSave">
				...
				<div class="row">
					...
					<div class="col-md-4">
						<dl class="dl-horizontal">
							...
							<dt style="font-size: 130%;">District</dt>
							<dd style="font-size: 130%;"><%=GetData.district_name(rs1.getInt("district"))%></dd>
						</dl>
						<!-- dl-horizontal -->
					</div>
					<!-- col-md-4 -->
		...
		<form name="updateform" action="<%= rootpath %>StockiestSave" method="post">
			<input type="hidden" name="id" value="<%= sid %>">
			<%
				...
				st=con.createStatement();
					 rs=st.executeQuery("SELECT *,DATE_FORMAT(druglicensevalidity, '%d-%m-%Y') as drug FROM stockiest INNER JOIN `stokiestcontact` ON `stokiestcontact`.`sid`=`stockiest`.`sid` WHERE `stockiest`.`sid`= '"+sid+"'");
					 while(rs.next())
					 {
				%>
				...
				<div class="row">
					<div class="col-md-4">
						...
						<div class="form-group">
							...
							<div class="row">
								<div class="col-md-6">
									<input type="text" class="form-control" name="cpname1"
								...
								<div class="col-md-6">
									<input type="text" class="form-control" id="contactnumber"
										name="cpphone1" placeholder="Enter Phone Number" autocomplete="off"
					...
					<div class="col-md-4">
						...
						<%Statement sthq=con.createStatement();
								ResultSet rshq=sthq.executeQuery("select daid, district from districts");
										%>
						<div class="form-group">
							<label>District</label> <select class="form-control select2"
								style="width: 100%;" name="headquarter">
								<%while(rshq.next()){ %>
								<option 
								<%=rs.getString("district").equals(rshq.getString("daid")) ? "selected='selected'" : "" %><%=rshq.getString("district")%>
								value="<%= rshq.getString("daid")%>"><%=rshq.getString("district")%>
									</option>
								<%} %>
							</select>
						</div>
