...
Connection con = C3P0DataSource.getInstance().getConnection();
	...
	ResultSet rs = null;
	rs = st.executeQuery("select name, sid from stockiest");
%>
				...
				<div class="box">
					...
					<div class="box-body">
						<table id="example1" class="table table-bordered table-striped">
							...
							<thead>
								<tr>
									<th><select class="form-control select2" multiple="multiple"
										name="sid" data-placeholder="Select Stockiest"
										style="width: 100%;">
											<% while(rs.next()){ %>
											<option value="<%=rs.getString("sid")%>"><%= rs.getString("name") %></option>
											<% } rs= st.executeQuery("select district from districts"); %>
									</select></th>
									
									<th><select class="form-control select2"
										style="width: 100%;" name="headquarter" multiple="multiple" data-placeholder="Select District">
							...
							String query1 = "select stockiest.*, districts.district as dname from stockiest inner join districts on districts.daid = stockiest.district where ";
							  String whr = "(";
							  String cquery = null;
								if(search != null){
							...
							<tbody>
								<% while(rs.next()){ %>
									<tr>
										<td><a method="post" href='Admin/UpdateStockiest.jsp?id=<%=rs.getInt("sid") %>'><%= rs.getString("name") %></a></td>
							...
							<% }else{ 
									cquery = "select stockiest.*, districts.district as dname from stockiest inner join districts on districts.daid = stockiest.district";
