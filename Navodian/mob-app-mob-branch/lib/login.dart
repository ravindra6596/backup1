...
class _LoginPageState extends State<LoginPage> {
  ...
  signIn(String username, password) async {
    ...
    var response = await http.post(
      Global().baseAPIurl + "/authenticate",
      body: body,
      headers: {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonData != null) {
        ...
        getUserData(username, jsonData['token']).then((empData) {
          if (empData != null) {
            sharedPreferences.setString("username", username);
            sharedPreferences.setString("name", empData['name']);
            sharedPreferences.setString("email", empData['email']);
            sharedPreferences.setInt("userid", empData['eid']);
          }
        });
  ...
  Future getUserData(username, token) async {
    //"http://healthcareapi-env.eba-83fpcz2g.ap-south-1.elasticbeanstalk.com/employee/username/"
    var empdata = await http.get(
      Global().baseAPIurl + "/employee/username/" + username,
      headers: {
        "Content-Type": "application/json",
        "accept": "application/json",
